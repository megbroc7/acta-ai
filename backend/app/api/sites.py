...
@router.post("/{site_id}/test-connection", response_model=ConnectionTestResponse)
async def test_existing_site_connection(
    site_id: int,
    connection_data: ConnectionTestRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Test connection to an existing WordPress site."""
    ...
    try:
        wp_service = WordPressService(
            api_url=connection_data.api_url,
            username=connection_data.username,
            app_password=connection_data.app_password or site.app_password
        )
...
async def update_wordpress_site(
    ...
    """Update an existing WordPress site connection."""
    # Check if site exists and belongs to user
    stmt = select(WordPressSite).where(
        (WordPressSite.id == site_id) &
        (WordPressSite.user_id == current_user.id)
    ...
    # If app_password is provided, test the connection
    connection_data = {
        "api_url": site_data.api_url,
        "username": site_data.username
    }
    
    # Only include app_password if it's provided
    if site_data.app_password:
        connection_data["app_password"] = site_data.app_password
    else:
        connection_data["app_password"] = site.app_password
    
    # Test the connection
    try:
        wp_service = WordPressService(**connection_data)
        connection_test = await wp_service.verify_connection()
        if not connection_test["success"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    site.api_url = site_data.api_url
    site.username = site_data.username
    
    # Only update app_password if provided
    if site_data.app_password:
        site.app_password = site_data.app_password
    ...
    # Fetch taxonomy data in the background
    background_tasks.add_task(
        fetch_and_save_taxonomies,
        db, site.id, wp_service
    )
    
    return site 

@router.post("/mock-test-connection", response_model=ConnectionTestResponse)
async def mock_test_connection():
    """Mock test connection for development and testing.
    Always returns success without actually connecting to a WordPress site."""
    return {
        "success": True,
        "message": "Mock connection successful! (No real WordPress site was contacted)",
        "user_info": {
            "id": 1,
            "name": "Mock WordPress Admin",
