...
const SiteDetail = () => {
  ...
  const fetchSite = useCallback(async () => {
    ...
    try {
      ...
      // Use the status from localStorage if available
      const siteWithConnectionStatus = {
        ...response.data,
        is_connected: savedStatus === 'true' // Convert string to boolean
      };
      
      setSite(siteWithConnectionStatus);
    } catch (err) {
  ...
  const testConnection = async () => {
    ...
    try {
      // Need to send required fields in the request body
      const requestData = {
      ...
      const response = await api.post(`/api/sites/${id}/test-connection`, requestData);
      ...
      if (response.data.success) {
        try {
          // Update the site in the database with is_connected status and all required fields
          await api.put(`/api/sites/${id}`, {
            name: site.name,
            url: site.url,
            api_url: site.api_url,
            username: site.username,
